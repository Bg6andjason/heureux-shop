...
type MapRange = (n: number, start: number, stop: number, start2: number, stop2: number) => number;

const mapRange: MapRange = (n, start, stop, start2, stop2) => {
...
class AsciiFilter {
  ...
  constructor(renderer: THREE.WebGLRenderer, { fontSize, fontFamily, charset, invert }: AsciiFilterOptions = {}) {
    ...
    // Non-standard legacy flags; safe no-op in modern browsers.
...
class CanvasTxt {
  ...
  constructor(
    txt: string,
    { fontSize = 200, fontFamily = 'Arial', color = '#fdf9f3' }: { fontSize?: number; fontFamily?: string; color?: string } = {}
  ) {
    this.canvas = document.createElement('canvas');
    const ctx = this.canvas.getContext('2d');
    if (!ctx) throw new Error('2D canvas context unavailable');
  ...
  clear() {
    this.scene.traverse(obj => {
      ...
      for (const mat of materials) {
        if (mat && typeof (mat as unknown as { dispose?: () => void }).dispose === 'function') {
          (mat as unknown as { dispose: () => void }).dispose();
        }
      }

      if (mesh.geometry && typeof (mesh.geometry as unknown as { dispose?: () => void }).dispose === 'function') {
